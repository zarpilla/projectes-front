      ...
      <card-component>
        <form @submit.prevent="submit2">
          <b-field horizontal>
            ...
            <!-- <b-field label="Ruta" class="is-narrow">
  ...
  async mounted() {
    ...
    this.filters.year = this.years.find(
      ...
      const contacts = await service({ requiresAuth: true }).get(
        "contacts?users_permissions_user=" + me.data.id
      ).then((r) => r.data);

      console.log(contacts);
      if (contacts.length === 0) {        
        this.$buefy.snackbar.open({
              message: "Error. No hi ha cap contacte associat a aquest usuari",
              queue: false,
              type: "is-danger"
            });
            return
      } else {
        this.filters.contact = contacts[0].id;
      }
